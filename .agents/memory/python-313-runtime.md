...
**How to apply:** Check the active Python version before testing. If 3.13 is unavailable, activate the supported `python-base-3.13` runtime rather than weakening the requirement or changing dependencies. Pin an available stable 3.13 interpreter explicitly; generic uv selection may choose an incompatible 3.14 beta. If `.replit` is intentionally kept unchanged, use the installed 3.13 binary explicitly with a temporary `UV_PROJECT_ENVIRONMENT` so uv does not select the stale 3.12 `.pythonlibs` environment. If greenlet cannot load, add the compiler's `libstdc++` directory to `LD_LIBRARY_PATH` for the test process only.